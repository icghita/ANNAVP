function varargout = ANNAVP(varargin)
% ANNAVP MATLAB code for ANNAVP.fig
%      ANNAVP, by itself, creates a new ANNAVP or raises the existing
%      singleton*.
%
%      H = ANNAVP returns the handle to a new ANNAVP or the handle to
%      the existing singleton*.
%
%      ANNAVP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ANNAVP.M with the given input arguments.
%
%      ANNAVP('Property','Value',...) creates a new ANNAVP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the ANNAVP before ANNAVP_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ANNAVP_OpeningFcn via varargin.
%
%      *See ANNAVP Options on GUIDE's Tools menu.  Choose "ANNAVP allows only one
%      instance to run (singleton)".
%
% See also: GUIDE, GUIDATA, GUIHANDLES

% Edit the above text to modify the response to help ANNAVP

% Last Modified by GUIDE v2.5 16-Sep-2020 13:01:42

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ANNAVP_OpeningFcn, ...
                   'gui_OutputFcn',  @ANNAVP_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ANNAVP is made visible.
function ANNAVP_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ANNAVP (see VARARGIN)

% Choose default command line output for ANNAVP
...
% UIWAIT makes ANNAVP wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ANNAVP_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in PLSPlotPushButton.
function PLSPlotPushButton_Callback(hObject, eventdata, handles)
% hObject    handle to PLSPlotPushButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles = guidata(handles.output);
oldpointer = get(handles.figure1, 'pointer');
...
selectedANN = loadedANN.ANNStorage(handles.SelectedANNIndex);
if(strcmp(selectedANN.NetworkType, 'Feedforward Neural Network'))
    figure('Name', 'PLS Regression Plot');
    plotPLSRegress(selectedANN.PlotData.PLSPlot{1}, selectedANN.PlotData.PLSPlot{2});
else
    h = msgbox('PLS Regression Plot is available only for Feedforward Neural Networks', 'Warning');
end
