function plotPLSRegress(inputs, outputs)
    if(iscell(inputs))
        ...
        for i=1:6
            ...
            subplot(2,2,1);
            plot(1:length(cumsum(100*PCTVAR(2,:))), cumsum(100*PCTVAR(2,:)), strcat('-', colors{i}, 'o')), hold on;
            xlabel('Number of PLS components');
            ylabel('Percent Variance Explained in Neutralization Data');

            yfitPLS = [ones(size(inputs{i}', 1), 1) inputs{i}']*betaPLS;   
            subplot(2,2,2);
            plot(outputs' ,yfitPLS, strcat(colors{i}, 'o')), hold on;
            xlabel('Observed Response');
            ylabel('Fitted Response');
            legend('R = 0.9995');

            subplot(2,2,3);
            plot(0:length(PLSmsep(2,:))-1, PLSmsep(2,:), strcat(colors{i}, '-o')), hold on;
